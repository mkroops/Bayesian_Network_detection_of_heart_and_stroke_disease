...
class NB_Classifier:
    ...
    def __init__(self, file_name, fitted_model=None):
        file_name = "stroke-data-discretized-test.csv"
    ...
    def read_data(self, data_file):
        ...
        return self.rand_vars , self.rv_all_values
    ...
    def compute_performance(self):
        ...
        for i in self.rv_all_values:
            ...
            evidence = {}
            for j in range(0, len(self.rand_vars)-1):
                evidence[self.rand_vars[j]] = i[j] 

            Bn.Likelihood_weighting(Bn, evidence)

            lw = i[0] + i[1] + i[len(self.rand_vars)-2]
            pred = predicted_output[lw][target_value]
            Y_prob.append(pred)
            print(lw, pred, target_value)
            best_key = max(predicted_output[lw] , key=predicted_output[lw].get)
